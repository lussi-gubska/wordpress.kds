...
add_action( 'wp_head', function () {
	...
	if ( $header_css ) {
		echo '<style id="penci-header-builder" type="text/css">' . $header_css . '</style>';
	}
}, 99 );
...
function penci_builder_customizer_css() {
	...
	$builder_slug = penci_get_header_builder_id();
	$builder_data = get_page_by_path( $builder_slug, OBJECT, 'penci_builder' );
	$builder_id   = isset( $builder_data->ID ) && $builder_data->ID ? $builder_data->ID : '';

	$custom_css = get_post_meta( $builder_id, 'penci_builder_css', true );
	...
	$main_menu_mgi         = penci_get_builder_mod( 'penci_header_pb_main_menu_penci_lv1_item_margin' );
	...
	$second_menu_mgi         = penci_get_builder_mod( 'penci_header_pb_second_menu_penci_lv1_item_margin' );
	...
	$third_menu_mgi         = penci_get_builder_mod( 'penci_header_pb_third_menu_penci_lv1_item_margin' );
	...
	$search_form_color = penci_get_builder_mod( 'penci_header_search_input_color' );
	...
	$login_font = penci_get_builder_mod( 'penci_header_pb_login_register_mobile_penci_font_login_text' );
	if ( ! empty( $login_font ) ) {
		$out .= 'font-family:' . penci_builder_get_font_data( $login_font ) . ';';
	}
	$login_uppercase = penci_get_builder_mod( 'penci_header_pb_login_register_mobile_text_uppercase' );
	if ( 'enable' == $login_uppercase ) {
		$out .= 'text-transform:uppercase;';
	}
	$login_weight = penci_get_builder_mod( 'penci_header_pb_login_register_mobile_penci_fontw_login_text' );
	if ( ! empty( $login_weight ) ) {
		$out .= 'font-weight:' . $login_weight . ';';
	}
	$out .= '}';
	...
	update_post_meta( $builder_id, 'penci_builder_css', $out . $return );
...
function penci_builder_get_area_css( $area, $cssprefix = '', $optionprefix = '', $customselect = '' ) {
	...
	$background_scolor     = 'mobile' == $cssprefix ? penci_get_builder_mod( 'penci_' . $optionprefix . '_' . $area . '_sticky_background_color' ) : '';
