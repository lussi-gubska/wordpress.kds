...
class PenciSoledadCodeStarOptions {
	...
	public function __construct() {
		...
		if ( ! is_admin() || get_theme_mod( 'penci_disable_theme_options' ) ) {
			return;
		}
	...
	public function add_options( $option_data ) {
		...
		$fields_data = array(
			...
			'title'   => isset( $option_data['label'] ) && $option_data['label'] ? $option_data['label'] : '',
			'desc'    => isset( $option_data['description'] ) && $option_data['description'] ? $option_data['description'] : '',
			'default' => isset( $option_data['default'] ) && $option_data['default'] ? $option_data['default'] : '',
		);
		...
		if ( isset( $option_data['active_callback'] ) && is_array( $option_data['active_callback'] ) ) {
			...
			foreach ( $option_data['active_callback'] as $callback ) {
				$fields_data['dependency'] = array( $callback['setting'], $callback['operator'], $callback['value'] );
			}
		}
...
new PenciSoledadCodeStarOptions();
