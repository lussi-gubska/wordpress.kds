...
class Scheduler {
  ...
  public static function schedule_post_id( $post_id ) {
    $args = [
      ...
      'hook'                => PENCI_PODCAST_IMPORTER_SCHEDULER_FEED_PREFIX . $post_id,
      'args'                => [ $post_id ],
      'group'               => PENCI_PODCAST_IMPORTER_SCHEDULER_FEED_GROUP
    ];

    $args = apply_filters( PENCI_PODCAST_IMPORTER_ALIAS . '_scheduler_feed_args', $args );
  ...
  public static function is_everything_scheduled() {
    ...
    foreach( $post_ids as $post_id )
      if( !self::is_action_scheduled( PENCI_PODCAST_IMPORTER_SCHEDULER_FEED_PREFIX . $post_id ) )
        return false;
