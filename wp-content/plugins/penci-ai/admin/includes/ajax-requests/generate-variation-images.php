...
class GenerateVariationImages {
	...
	public function __construct( $a ) {
		...
		add_action( "wp_ajax_penciai_generate_variation_images", [ $this, 'ajax' ] );
	}
	...
	public function ajax() {
		...
		$engine         = isset( $_POST['ai-image-engine'] ) ? sanitize_key( $_POST['ai-image-engine'] ) : "dall_e";
		...
		if ( ! empty( get_theme_mod( 'penci_ai_api_key' ) && 'dall_e' == $engine ) ) {
			...
			if ( empty( $image_experiments ) ) {
				$image_experiments = get_theme_mod( 'penci_ai_img_experiments', array() );
			}
			...
			if ( isset( $message->upscaled_photo_url ) ) {
				$img_lists[] = [ 'url' => $message->upscaled_photo_url ];
				wp_send_json_success( $img_lists );
			} else {
				wp_send_json_error( 'Can\'t get data from Discord channel.', 'soledad' );
			}
		} elseif ( $this->image_token ) {
