	...
	class Penci_Soledad_Portfolio_Shortcode {
		...
		function portfolio_shortcode( $atts, $content = null ) {
			...
			extract( shortcode_atts( array(
				...
				'cat'             => '',
				'filter'          => 'true',
				'all_text'        => __( 'All', 'penci-portfolio' ),
				'pagination'      => 'number',
				'numbermore'      => 6,
				'loop'            => '',
				'penci_ajax'      => '',
			...
			if ( empty( $all_text ) ): $all_text = __( 'All', 'penci-portfolio' ); endif;
			...
			$wrap_cls  = $penci_ajax ? 'pcpt-ajax-ftl' : 'pcpt-normal-ftl';
			ob_start();
			/* Portfolio Filter */
			?>
            <div id="<?php echo esc_attr( $unique_id ); ?>" class="wrapper-penci-portfolio <?php echo $wrap_cls; ?>">
				<?php if ( $filter == 'true' ):
					?>
                    ...
                    <div class="penci-portfolio-filter">
                        <ul>
                            <li class="all active">
                                <a data-term="*" data-filter="*" href="#" class="alltext"><?php echo $all_text; ?></a>
                            </li>
							<?php
							$term_arrayss = array();
                ...
                <div id="<?php echo $unique_id; ?>"
                     class="penci-portfolio penci-portfolio-wrap column-<?php echo $column; ?> penci-portfolio-<?php echo $item_style . ' ' . $wrap_cls; ?>">
                    <div class="inner-portfolio-posts">
						<?php while ( $portfolio_query->have_posts() ): $portfolio_query->the_post(); ?>
                            ...
                            <article class="<?php echo $item_classes; ?>" id="portfolio-<?php the_ID(); ?>"
                                     data-pflID="<?php the_ID(); ?>"
                                     data-terms="<?php echo implode( ' ', $tfl_item_terms ); ?>">
                                        ...
                                        <div class="penci-portfolio-thumbnail">
                                            ...
                                            <a href="<?php the_permalink(); ?>">
                                                ...
                                                <a class="penci-portfolio-open-lightbox lightbox-port-thumb"
                                                   ...
                                                   href="<?php echo penci_get_featured_image_size( get_the_ID(), 'penci-single-full' ); ?>">
													<?php } ?>
													<?php
													Penci_Portfolio::get_image_ratio( get_the_ID(), $image_thumb, $padding );
													?>
                                                </a>
                                        </div>
                ...
                <div class="penci-pagenavi-shortcode">
					...
					if ( in_array( $pagination, array( 'load_more', 'infinite' ) ) ) {
						...
						<?php
					} else if ( function_exists( 'penci_pagination_numbers' ) && ( 'none' != $pagination ) ) {
...
if ( ! function_exists( 'penci_pfl_more_post_ajax_func' ) ):
	...
	function penci_pfl_more_post_ajax_func() {
		...
		$image_thumb = 'penci-masonry-thumb';
		$padding     = false;
		if ( $style == 'grid' ): $image_thumb = 'penci-thumb';
			$padding                          = '66.6666667'; endif;

		$image_thumb_pre = '';
		if ( $style == 'grid' && 'square' == $image_type ) {
			$image_thumb_pre = 'penci-thumb-square';
			$padding         = '100';
		} elseif ( $style == 'grid' && 'vertical' == $image_type ) {
			$image_thumb_pre = 'penci-thumb-vertical';
			$padding         = '135.4';
		}
		...
		while ( $portfolio_query->have_posts() ): $portfolio_query->the_post(); ?>
			...
			?>
            <article class="<?php echo $item_classes; ?>" id="portfolio-<?php the_ID(); ?>"
                     data-pflID="<?php the_ID(); ?>" data-terms="<?php echo $tfl_item_terms; ?>">
                <div class="inner-item-portfolio">
                    <div class="info-portfolio">
                        <div class="penci-portfolio-thumbnail">
							<?php if ( $lightbox != 'true' || ! function_exists( 'penci_get_featured_image_size' ) ) { ?>
                            <a href="<?php the_permalink(); ?>">
								<?php } else { ?>
                                <a class="penci-portfolio-open-lightbox lightbox-port-thumb"
                                   title="<?php esc_attr( the_title() ); ?>" data-rel="penci-gallery-image-content"
                                   href="<?php echo penci_get_featured_image_size( get_the_ID(), 'penci-single-full' ); ?>">
									<?php } ?>
									<?php
									Penci_Portfolio::get_image_ratio( get_the_ID(), $image_thumb, $padding );
