...
namespace PenciPaywall\Payments\Stripe\Lib\Http_Client;

use PenciPaywall\Payments\Stripe\Lib\Stripe;
use PenciPaywall\Payments\Stripe\Lib\Util;
use PenciPaywall\Payments\Stripe\Lib\Exception\Api_Connection_Exception;
...
class Curl_Client implements Client_Interface {
	...
	public function __construct( $defaultOptions = null, $randomGenerator = null ) {
		...
		$this->randomGenerator = $randomGenerator ?: new Util\Random_Generator();
		...
		while ( true ) {
			...
			if ( is_callable( $this->getRequestStatusCallback() ) ) {
				call_user_func_array(
					...
					array( $rbody, $rcode, $rheaders, $errno, $message, $shouldRetry, $numRetries )
				);
			}
	 ...
	 * @throws Exception\Api_Connection_Exception
	 */
	private function handleCurlError( $url, $errno, $message, $numRetries ) {
		...
		throw new Api_Connection_Exception( $msg );
	}
