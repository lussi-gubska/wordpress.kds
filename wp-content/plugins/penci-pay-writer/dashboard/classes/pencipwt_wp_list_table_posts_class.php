...
class PenciPWT_Posts_List_Table extends WP_List_Table {
	...
	function __construct( $stats_data ) {
		...
		$this->perm = new PenciPWT_Permissions();
		...
		if ( isset( $item[ $column_name ] ) ) {
			...
			switch ( $column_name ) {
				...
				case 'post_title':
					...
					if ( pencipwt_get_setting('stats_display_edit_post_link') ) {
				...
				case 'post_comments':
					...
					if ( $post->ppc_count['normal_count'][ $label_field_value ]['real'] != $post->ppc_count['normal_count'][ $label_field_value ]['to_count'] ) {
						$field_value = '<abbr title="' . sprintf( __( 'Total is %1$s. %2$s Displayed is what you\'ll be paid for.', 'penci-pay-writer' ), $post->ppc_count['normal_count'][ $label_field_value ]['real'], '&#13;' ) . '" class="ppc_count_column">' . $item[ $column_name ] . '</abbr>';
					}
			...
			if ( ! isset( $field_value ) ) {
				$field_value = apply_filters( 'ppc_author_stats_each_field_empty_value', '0', $column_name ) . '</td>';
			}
		}
